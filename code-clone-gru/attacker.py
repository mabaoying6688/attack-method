...
class CombinedAttacker(object):
    ...
    def _attack_ins(self,x_token, x, x2, y, poses,rand_d, n_candidate=100, n_iter=20, relax=1):
        ...
        print ("FAIL!")
        return False, x, x2, y
    def attack_all11(self, n_candidate=100, n_iter=20, relax=1, res_save=None):
        count_label_0 = 0  
        count_label_1 = 0  
        ...
        for i in range(self.d.test.get_size()):
            ...
            if n_succ <= 0:
                ...
                new_data = pd.DataFrame({
                    ...
                    'call times':[(GRUClassifier.counter+GRUEncoder.counter1)], 
                    'Aver call times':[(GRUClassifier.counter+GRUEncoder.counter1)/n_succ]
                    })
                loss_df = pd.concat([loss_df, new_data], ignore_index=True)
                loss_df.to_csv('GRU-clone-COM+B-succ_values.csv', index=False)  
...
if __name__ == "__main__":
    ...
    classifier.load_state_dict(torch.load('../model/model.pt'))

  
    attacker = CombinedAttacker(poj, symtab, instab, classifier)
    attacker.attack_all(3, 50)

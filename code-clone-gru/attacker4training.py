...
from lstm_classifier import  GRUClassifier, GRUEncoder
from modifier import TokenModifier, InsModifier
from sklearn.metrics import precision_recall_fscore_support
from dataset import remove_tail_padding
import numpy as np
import random
import time
import torch
...
#AdversarialTrainingAttacker_Random
        ...
        while iter < n_iter:
            ...
            for k in keys:
                ...
                if k in self.tokenM.forbidden_uid:
                    ...
                    continue
                iter += 1
                new_x, new_uid_cand = self.tokenM.rename_uid(x, x2, y, uids[k], k,vocab_cnt_test, n_candidate)
    ...
    def attack_all(self, n_candidate=100, n_iter=20, relax=1, res_save=None, adv_sample_size=5000):
        ...
        for i in range(self.d.train.get_size()):
            ...
            for _ in range(30):
                ...
                rand_d.append(rand)
            # print("rand_d",b['raw1'])
            tag, x, x2, typ = self.attack(b['raw1'],b['x1'], b['x2'], b['y'], self.syms['tr'][b['id1'][0]], self.inss['stmt_tr'][b['id1'][0]],
        ...
        if res_save is not None:
            ...
            with gzip.open(res_save, "wb") as f:
                unpadding_adv_x1s = [remove_tail_padding(adv_x, 0) for adv_x in adv_x1s]
                unpadding_adv_x2s = [remove_tail_padding(adv_x, 0) for adv_x in adv_x2s]
                pickle.dump({"fail_pred_x1s": fail_pred_x1s, 
                             "fail_pred_x2s": fail_pred_x2s, 
                             "fail_pred_labels": fail_pred_labels,
                             "adv_raw1": self.d.idxs2raw(unpadding_adv_x1s, [len(x) for x in unpadding_adv_x1s]),
                             "adv_raw2": self.d.idxs2raw(unpadding_adv_x2s, [len(x) for x in unpadding_adv_x2s]),
                             "adv_x1s": adv_x1s,
                             "adv_x2s": adv_x2s,
...
if __name__ == "__main__":
    ...
    classifier.load_state_dict(torch.load('../model/PATH15.pt'))


    attacker = CombinedAttacker(poj, symtab, instab, classifier)
    attacker.attack_all(40, 50,1, res_save="codeclone_GRU.advsamples.pkl.gz", adv_sample_size=2000)

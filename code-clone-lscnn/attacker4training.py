...
class CombinedAttacker(object):
    ...
    def attack(self,x, x2, y, uids,poses,rand_d,vocab_cnt_test=None,n_candidate=100, n_iter=20,relax=1):
        x_token=x
        ins_success,x3, x4,ins_adv_y = self._attack_ins(x_token,x,x2, y, poses, rand_d, n_candidate, n_iter,relax=1)
        if ins_success:
            return True,x3, x4, ins_adv_y
        token_success,x5,x6,token_adv_y = self._attack_token(x,x2, y, uids, vocab_cnt_test, n_candidate, n_iter)
        if token_success:
            return True,x5,x6,token_adv_y
    ...
    def _attack_token(self, raw1, raw2, y, uids,vocab_cnt_test, n_candidate=100, n_iter=20):
        ...
        while iter < n_iter:
            ...
            for k in keys:
                ...
                for uid, p, pr,x_ in zip(new_x_uid, new_pred, new_prob,new_raw1):
                    if p != y:
                        print ("SUCC!\t%s => %s\t\t%d(%.5f) => %d(%.5f) %d(%.5f)" % \
                               (k, self.idx2txt[uid], y, old_prob, y, pr[y], p, pr[p]))
                        return True,x_, new_raw2[0], 1
    ...
    def attack_all(self, n_candidate=100, n_iter=20, relax=1, res_save=None, adv_sample_size=5000):
        ...
        for i in range(self.d.train.get_size()):
            ...
            print ("\t%d/%d\tID = (%d, %d)\tY = %d" % (i+1, self.d.train.get_size(), b['id1'][0], b['id2'][0], b['y'][0]))
        ...
        # Generate rand_d for ins attack
            ...
            print("b['raw1'][0]",b['raw1'][0])
            tag, x, x2, typ = self.attack(b['raw1'][0],b['raw2'][0], b['y'][0], self.syms['tr'][b['id1'][0]], self.inss['stmt_tr'][b['id1'][0]],
            ...
            if typ==1 or tag == 1:
                ...
                for item in x:
                    if isinstance(item, list):  
                        item = tuple(item)
                    if item in self.txt2idx:
                        result.append(self.txt2idx[item])
                result1 = []
                for item in x2:
                    if isinstance(item, list):  
                        item = tuple(item)
                    if item in self.txt2idx:
                        result1.append(self.txt2idx[item])
                target_length = 300
                if len(result) < target_length:
                    result = np.pad(result, (0, target_length - len(result)), mode='constant', constant_values=0)
                else:
                    result = result[:target_length]  
                if len(result1) < target_length:
                    result1 = np.pad(result1, (0, target_length - len(result1)), mode='constant', constant_values=0)
                else:
                    result1 = result1[:target_length]                  
                x= result
                x2= result1 

            if tag==True and np.any(x):
                n_succ += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="../model/49.pt")
    ...
    attacker.attack_all(20, 10,1, res_save="../codeclone_lscnn.advsamples.pkl.gz", adv_sample_size=500)

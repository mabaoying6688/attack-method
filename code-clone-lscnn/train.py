...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default="../code_clone-lscnn/model")
    parser.add_argument('--bs', type=int, default=32)
    parser.add_argument('--nepoch', type=int, default=50)   
    #parser.add_argument('--adv_train_path', type=str, default=None)
    #parser.add_argument('--adv_train_size', type=int, default=240)   
    ...
    test_set = poj.test   
    training_set = poj.train
    training_set.reset_epoch()  
    batch_size = 1  
    total_samples = training_set.get_size()
    print(f"total_samples: {total_samples}")
    for i in range(total_samples):
        batch = training_set.next_batch(batch_size)
        if batch['raw1'][0] is not None:
            print(batch['raw1'][0])                
        if batch['raw2'][0] is not None:
            print(batch['raw2'][0])               
    print("===== Finish=====")

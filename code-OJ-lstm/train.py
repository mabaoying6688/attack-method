...
def evaluate(dataset, batch_size=128):  
    ...
    while True:        
        ...
        with torch.no_grad():
            outputs = classifier(inputs)[0]
            #print(outputs.size())#torch.Size([128, 104])
            res = torch.argmax(outputs, dim=1) == labels
            #print(res.size())torch.Size([128])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-save_dir', default="../model/new")
    parser.add_argument('-model', default="LSTM")
    parser.add_argument('-attn', action='store_true')
    parser.add_argument('-l2p', type=float, default="0.5")
   ...
   # print("opt.attn: ",opt.attn)  
    ...
    if opt.model == 'LSTM' or opt.model == 'GRU':
        ...
        ## TODO: 
        #optimizer = optim.SGD(classifier.parameters(), lr=3.625, weight_decay=0.5)
    else:
...
#             torch.optim.Adam(classifier.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
    ...
    trainEpochs(100, training_set, valid_set, saving_path=opt.save_dir)
    print()
    print()

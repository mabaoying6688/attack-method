...
def build_vocab(codes):   
    ...
    idx2txt_test = ["<unk>"] + [it[0] for it in vocab_cnt]
    '''
    xx = [it[0] for it in vocab_cnt]
    for item in xx:
     print(item)
     '''
    txt2idx_test = {}
    for idx in range(len(idx2txt_test)):
        txt2idx_test[idx2txt_test[idx]] = idx
    return idx2txt_test, txt2idx_test,vocab_cnt

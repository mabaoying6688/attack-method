...
def InsResult1(tokens, posi, insertDict):
    result = []
    insert_tokens = []
    if insertDict.get(posi) is not None:
        for tokenList in insertDict[posi]:
            insert_tokens += tokenList
    ...
    for i in range(len(tokens) - 1):
        if tokens[i] == "main" and tokens[i + 1] == "(":
            ...
            break
    if main_start == -1:
        return insert_tokens + list(tokens)
    result = list(tokens[:main_start]) + insert_tokens + list(tokens[main_start:])
    return result
    ...
    for i, stmt in enumerate(stmts):
        if stmt.rstrip().endswith("{"):
            ...
            if stmt.lstrip().startswith("struct") or stmt.lstrip().startswith("union"):
                structStack.append((stmt, paren_n))#stmt、paren_n入栈

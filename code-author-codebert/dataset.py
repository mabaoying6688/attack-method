    ...
    def __init__(self, xs=[], ys=[], raws=None, ids=None, idx2txt=[], txt2idx={},
        ...
        for x, y, r, i in zip(xs, ys, raws, ids):
            ...
            if len(x) > 0 and isinstance(x[0], list):
                 x = [token for sub in x for token in sub]
            # print("x=",x)

            for t in x:
                if isinstance(t, str):
                    t = self.__txt2idx.get(t, self.__txt2idx['<unk>'])
                # print("t=",t)
...
class Author66(object):
    def __init__(self, path='../data_author/author.pkl.gz',
                 vocab_size=-1, dtype='32', adv_train_path=None, adv_train_size=None):
        ...
        for i, t in zip(range(self.__vocab_size), self.__idx2txt):
            self.__txt2idx[t] = i
        self.train = Dataset(xs=d['x_tr'][:528], ys=d['y_tr'][:528], raws=d['raw_tr'][:528],
                             idx2txt=self.__idx2txt, txt2idx=self.__txt2idx,
                             vocab_size=self.__vocab_size, dtype=self.__dtypes)

        self.test = self.dev = Dataset(xs=d['x_te'][:132], ys=d['y_te'][:132], raws=d['raw_te'][:132],
                                       idx2txt=self.__idx2txt, txt2idx=self.__txt2idx,
                                       vocab_size=self.__vocab_size, dtype=self.__dtypes)
        ...
        if adv_train_path is not None:
            ...
            print("[Adversarial Training] adversarial sample number", flush=True)

...
class CombinedAttacker(object):    
    ...
    def _attack_token(self, x, y, uids, vocab_cnt_test, n_candidate=100, n_iter=20):
        ...
        if torch.argmax(old_prob) != y[0]:
            ...
            return True, x, 0
#[torch.argmax(old_prob).cpu().numpy()]
    ...
    def attack_all(self, n_candidate=100, n_iter=20, res_save=None, adv_sample_size=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            tag, x, typ = self.attack(b['raw'],b['x'], b['y'], self.syms['te'][b['id'][0]], self.inss['stmt_te'][b['id'][0]], rand_d, vocab_cnt_test,n_candidate, n_iter)
            x = x[0]  
            print("x =====",x)  
            print("b['y'][0] =====",b['y'][0])  
            print("b['id'][0] =====",b['id'][0])  
        ...
        print("[Task Done] Time Cost: %.1f sec Succ Rate: %.3f Aver call times: %.3f" % \
              (time.time()-st_time, n_succ/self.d.test.get_size(), (LSTMClassifier.counter + LSTMEncoder.counter1)/n_succ))
...
if __name__ == "__main__": 
    ...
    enc = LSTMEncoder(embedding_size, hidden_size, n_layers, drop_prob=0.3, brnn=True)
    classifier = LSTMClassifier(
        vocab_size, embedding_size, enc, hidden_size, num_classes, max_len,
        dropout_p=0.3, attn=True
    ).cuda()   
    classifier.load_state_dict(torch.load('../model/author_lstm/best_model.pt'))
    #=========================== attack after adv-training =========================

    attacker = CombinedAttacker(poj, symtab, instab, classifier)
    attacker.attack_all(10, 20,res_save="../lstm_author_atk.advsamples.pkl.gz", adv_sample_size=300)

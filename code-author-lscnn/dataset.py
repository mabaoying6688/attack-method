...
class Author66(object):
    ...
    def __init__(self, path='../data_author/author.pkl.gz', max_stmt_len=20, max_stmt_cnt=40,
                 vocab_size=5000, valid_ratio=0.2, dtype='32',adv_train_path=None, adv_train_size=None):
        ...
        x_train = d['x_tr'][:528]
        y_train = d['y_tr'][:528]
        raw_train = d['raw_tr'][:528]

        if adv_train_path != None:
            ...
            if adv_train_size != None:
                tmp_idxs = random.sample(range(len(adv_x)), min(adv_train_size, len(adv_x)))
                adv_x = [adv_x[i] for i in tmp_idxs]
                adv_y = [adv_y[i] for i in tmp_idxs]
            

            adv_x_indices = []
            for text_seq in adv_x:
                idx_seq = []
                for token in text_seq:
                    if token in self.__txt2idx:
                        idx_seq.append(self.__txt2idx[token])
                    else:
                        idx_seq.append(self.__txt2idx['<unk>'])
                adv_x_indices.append(idx_seq)
            

            x_train = list(x_train) + adv_x_indices
            y_train = list(y_train) + list(adv_y)
            raw_train = list(raw_train) + [None] * len(adv_x)
            
    # print("[Adversarial Training] adversarial sample number: %d" % len(adv_x), flush=True)   
        self.train = Dataset(xs=x_train,
                            ys=y_train,
                            raws=raw_train,
...
if __name__ == "__main__":
    ...
    oj = Author66(path="../data_author/author.pkl.gz")

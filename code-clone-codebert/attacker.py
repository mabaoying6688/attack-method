...
class CombinedAttacker(object):
    ...
    def attack(self,x_raw,x_raw2,x, x2, y, uids,poses,rand_d,vocab_cnt_test=None,n_candidate=100, n_iter=20):
        ...
        token_success,prep= self._attack_token(x_raw[0],x_raw2[0], y, uids, vocab_cnt_test, n_candidate, n_iter)
        if token_success:
            return True,prep            
        return False, y
    ...
    def _attack_token(self, x, x2, y, uids, vocab_cnt_test, n_candidate=100, n_iter=20):
        ...
        # print("torch.argmax(old_prob) != y[0]=",torch.argmax(old_prob) != y[0])
    ...
    def attack_all(self, n_candidate=100, n_iter=20, dump_samples_path=None):
        ...
        for i in range(self.d.test.get_size()):
            ...
            tag, pred = self.attack(b['raw1'],b['raw2'],b['x1'], b['x2'], b['y'], self.syms['te'][b['id1'][0]], self.inss['stmt_te'][b['id1'][0]],
            ...
            if n_succ <= 0:
                ...
                loss_df.to_csv('CodeBERT_oj-tt信息熵5-4-succ_values.csv', index=False)

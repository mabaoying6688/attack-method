...
def get_batched_data(raws1, raws2, ys, txt2idx, ids1=None, ids2=None):

    xs1 = raw2x(raws1, txt2idx)
    xs2 = raw2x(raws2, txt2idx)

    batch = {
        "x1": [" ".join(x) for x in xs1],   
        "x2": [" ".join(x) for x in xs2],
        "y": ys,
        "raw1": deepcopy(raws1),
        "raw2": deepcopy(raws2),
        "id1": ids1,
...
class TokenModifier(object):
    ...
    def rename_uid(self, uids, x, x2, y, x_uid, vocab_cnt_test, ori_uid, n_candidate=5):
        _x_uid = []
        for i in x_uid:

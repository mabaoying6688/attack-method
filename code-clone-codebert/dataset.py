...
class Dataset(object):
    ...
    def next_batch(self, batch_size=32):
        ...
        for idx1, idx2 in idx_pairs:
            #print(f"idx1: {idx1}")
            #print(f"self.__seqs: {len(self.__seqs)}")
        ...
        batch['code1'] = [" ".join(raw) if isinstance(raw, list) else str(raw) for raw in batch['raw1']]
        batch['code2'] = [" ".join(raw) if isinstance(raw, list) else str(raw) for raw in batch['raw2']]
        return batch
    ...
    def __init__(self, path='../data_clone/oj.pkl.gz', max_len=500, vocab_size=2000,
        ...
        if adv_train_path != None:  # add adversarial samples for training
            ...
            if adv_train_size != None:
                ...
                tmp_idxs = random.sample(range(len(adv_x1)), min(3000, len(adv_x1)))
            ...
            adv_x1_idx = []
            adv_x2_idx = []
            for seq1, seq2 in zip(adv_x1, adv_x2):
                seq1_idx = [self.vocab2idx(token) for token in seq1 if isinstance(token, str)]
                seq2_idx = [self.vocab2idx(token) for token in seq2 if isinstance(token, str)]
                adv_x1_idx.append(seq1_idx)
                adv_x2_idx.append(seq2_idx)            
            adv_x1, adv_x2 = adv_x1_idx, adv_x2_idx            

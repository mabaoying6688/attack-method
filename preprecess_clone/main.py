...
def build(data_path="../data/oj.pkl.gz",
    ...
    with gzip.open(data_path, "rb") as f:
        d = pickle.load(f)    
    output_oj_txt_path = "oj-pkl.txt"
    with open(output_oj_txt_path, "w", encoding="utf-8") as output_file:
    ...
    for _uids in train_uid:
        for _uid in _uids.keys():
            if _uid not in all_uid:
                all_uid.append(_uid)
    idx2txt, txt2idx,vocab_cnt_tr = bcd.build_vocab(train_raw)
    idx2txt_test, txt2idx_test,vocab_cnt_test = bcd.build_vocab(test_raw) 

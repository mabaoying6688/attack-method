    ...
    def _attack_ins(self,x_token, x, x2, y, poses,rand_d, n_candidate=100, n_iter=20, relax=1):
        ...
        while iter < n_iter:
            ...
            try:    
                ...
                print(f"Error in merging candidates: {e}")
                print(f"Current sample info:")
                print(f"x: {x}, x2: {x2}, y: {y}")
                print(f"new_x_del: {new_x_del} (length: {len(new_x_del)})")
                print(f"new_x_add: {new_x_add} (length: {len(new_x_add)})")
                print(f"new_x2_del: {new_x2_del} (length: {len(new_x2_del)})")
                print(f"new_x2_add: {new_x2_add} (length: {len(new_x2_add)})")
                return False, x, x2, y
            # print("new_x=", new_x)
            # print("new_x2=", new_x2)
...
if __name__ == "__main__":
    ...
    attacker.attack_all(10, 10,1, "./lstm_atk.advsamples.pkl.gz", adv_sample_size=5000)

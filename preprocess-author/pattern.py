...
def tokens2stmts(tokens):
    ...
    for stmt in stmts:
        if stmt != "":
            ...
            newStmts.append(_getIndent(matchStmt)+"pass")  
...
def StmtInsPos(tokens, strict=True):
    ...
    n = len(statements)
    for i, (stmt, indent) in enumerate(zip(statements, indents)):
        cnt += len(stmt)
        if len(stmt) == 0:
            continue
        res.append(cnt - 1)
...
def DeclInsPos(tokens):
    ...
    res = [-1]  
    cnt = 0
    for stmt, indent in zip(statements, indents):
        cnt += len(stmt)
        if len(stmt) == 0:
            continue
        if all(t == "pass" for t in stmt):
            continue
        res.append(cnt - 1)
    return res
...
def FuncInsPos(tokens):
    ...
    for stmt, indent in zip(statements, indents):
        ...
        if len(stmt) == 0:
            continue
        if len(indent) == 0:
            if not (stmt[-1] == ":" and stmt[0] in ["def", "class"]):
                res.append(cnt - 1)
    return res
